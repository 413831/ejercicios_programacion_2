...
namespace _20180628_SP.v1
{
    ...
    {
        Dao<Votacion> dao;
        XmlSerializar<Votacion> xml;
        Votacion votacion;
        Thread hilo;
        Dictionary<string, Votacion.EVoto> participantes;
        List<PictureBox> graficos;
        String rutaArchivo = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\votaciones.xml";
        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    // Guardar resultados
                    xml.Guardar(rutaArchivo, this.votacion);
                    dao.Guardar(null, this.votacion);
